		...
		const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (mtcp->smap[sockid].socktype != MTCP_SOCK_STREAM && 
		...
		TRACE_API("Not a stream or datagram socket id: %d\n", sockid);
		errno = ENOTSOCK;
		return -1;
	}
	...
	/*UDP Specific */
	if(mtcp->smap[sockid].socktype == MTCP_SOCK_DGRAM){
		//make entry in udp_flow_table
		struct four_tuple_key* k = (struct four_tuple_key*) calloc(1,sizeof(struct four_tuple_key));
		k->sip = addr_in->sin_addr.s_addr;
		k->sport = addr_in->sin_port;
		//mtcp->mudp_flow_table[*k] = &mtcp->udp_smap[sockid];
		mtcp->mudp_flow_table.insert(make_pair(*k,&mtcp->smap[sockid]));
...
/*----------------------------------------------------------------------------*/
int
CloseDgramSocket(mctx_t mctx, int sockid){
	...
	//dense_hash_map<struct four_tuple_key,struct udp_socket *,four_tuple_hash,four_tuple_eq> :: iterator it = mtcp->mudp_flow_table.find(k);
	struct sockaddr_in *addr_in = &mtcp->smap[sockid].saddr;
	struct four_tuple_key k;
	k.sip = addr_in->sin_addr.s_addr;
	k.sport = addr_in->sin_port;

	//unordered_map<struct four_tuple_key,struct socket_map *,four_tuple_hash,four_tuple_eq> :: iterator it = mtcp->mudp_flow_table.find(k);
	dense_hash_map<struct four_tuple_key,struct socket_map *,four_tuple_hash,four_tuple_eq> :: iterator it = mtcp->mudp_flow_table.find(k);
	if(it != mtcp->mudp_flow_table.end()){
		mtcp->mudp_flow_table.erase(it);
	}
	//Empty socket recv list.
...
mtcp_close(mctx_t mctx, int sockid)
{
	...
	case MTCP_SOCK_DGRAM:
		ret = CloseDgramSocket(mctx, sockid);
		break;
