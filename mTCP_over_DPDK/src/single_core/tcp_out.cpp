		...
		uint32_t cur_ts, uint32_t echo_ts)
{
	...
	struct rte_mbuf *parent = (struct rte_mbuf *)IPOutputStandalone(mtcp, IPPROTO_TCP, 0,saddr, daddr, TCP_HEADER_LEN + optlen + payloadlen);
	...
	int pkt_data_len = TCP_HEADER_LEN + optlen + payloadlen + ETHERNET_HEADER_LEN + IP_HEADER_LEN;
		...
		struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
	...
	last = TAILQ_LAST(&sender->control_list, mtcp_sender::control_head);
	while (cur_stream) {
		if (++cnt > thresh)
			break;
		...
		struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
	...
	last = TAILQ_LAST(&sender->send_list, mtcp_sender::send_head);
	while (cur_stream) {
		if (++cnt > thresh)
			break;
		...
		struct mtcp_sender *sender, uint32_t cur_ts, int thresh)
{
	...
	last = TAILQ_LAST(&sender->ack_list, mtcp_sender::ack_head);
	while (cur_stream) {
		if (++cnt > thresh)
			break;
...
GetSender(mtcp_manager_t mtcp, tcp_stream *cur_stream)
{
	if (cur_stream->sndvar->nif_out < 0) {
		//cout << "Returning Global sender." << endl;
		...
		//cout << "Returning Other sender." << endl;
